...
#Example for BS data:
#bash ../proto.pipeline.v8.sh -i ./ -o ./output_CB -p GGCGGAAAGCACATCTGC -T 14 -a

#Example for LJ data:
#bash ../proto.pipeline.v8.sh -i ./ -o ./output_CB -p GGGAAAGCGA -q AGAAAAACGG -T 14 -a -e "-L 5"

#Example for EJ:
#bash ../proto.pipeline.v8.sh -i ./ -o ./output_CB -p CTACGAATTC -q CTGCAGTGAA -T 14 -a -e "-C completely_miss_the_point:0"


usage="USAGE: bash proto.pipeline.v8.sh -i [-o -p -q -r -T -h -a -e]
where:
	REQUIRED
...
#test to verify panaseq is installed and can be found
...
#################################################### CREATE HISTO FILE FOR AA
	...
	#awk 'NR > 3 {print length($1)}' ${file//_counts.txt}'_counts.aa.txt' > ${file//_counts.txt}'_counts.aa_histo_test.txt'
